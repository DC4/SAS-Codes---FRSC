...
**********************************************************
		LR_Country_Itunes_Apple_Test TW Debit
...
CR.falt002_TW_db_APR2021 
CR.falt002_TW_db_MAY2021
...
( AUTH_DECISION_XCD  = "A" OR (AUTH_DECISION_XCD = "" AND  DECI_CD_ORIG  = "A")) AND
 TRN_TYP  IN ("C" "M" "P") AND
(TRN_POS_ENT_CD  IN ("E" "K" "G") OR AUTH_DECISION_XCD = "") AND
SUBSTR(USR_IND_4,1,2) NE "Y2" AND
(
INDEX(UPCASE(MER_NM),"ITUNES") OR
INDEX(UPCASE(MER_NM),"APPLE")
) AND
CUM_AMOUNT_CNP_DAILY > 1000 AND
...
INC.falt003_db_04JUL;
       WHERE UPCASE(RULE_NAME_STRG) IN ('LR_COUNTRY_ITUNES_APPLE_TEST') and CLIENT_XID IN  ("SC_TANDEMTW_DB")
and date1 GE "03JUL2021"D;
RUN;

PROC SORT DATA = RULE_DATA NODUPKEY; BY RULE_NAME_STRG FI_TRANSACTION_ID; RUN;

PROC FREQ DATA = TXN_DATA;TITLE "LR_Country_Itunes_Apple_Test  SAS";
TABLE CRD_CLNT_ID*DATE1/NOCOL NOROW NOPERCENT NOCUM; RUN; 

PROC FREQ DATA = RULE_DATA; TITLE "LR_Country_Itunes_Apple_Test  FALCON";
TABLE CLIENT_XID*DATE1/NOCOL NOROW NOPERCENT NOCUM; RUN; 

/* Writing Outout to Excel - For Audit*/

ods listing;
ods excel file='C:\Users\1510806\Desktop\SAS\Outputs\LR_Country_Itunes_Apple_Test TW Debit.xlsx'
...
outfile= "C:\Users\1510806\Desktop\SAS\Outputs\LR_Country_Itunes_Apple_Test TW Debit.xlsx"
dbms=xlsx replace;
sheet="TXN_DATA";
run;
