...
*************************************************************************
		MR_Global_STEAMGAMES_Non_Major_Test Global Debit
...
(INDEX(UPCASE(MER_NM), "STEAMGAMES") OR INDEX(UPCASE(MER_NM), "STEAMPOWERED")) AND
...
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWGM_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWLK_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWZW_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWJO_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWTZ_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)
...
OR
(
 CRD_CLNT_ID  = "SC_SPARROWNP_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWSL_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)
...
OR
(
 CRD_CLNT_ID  = "SC_EURONETVN_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWCI_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_SPARROWCM_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)

OR
(
 CRD_CLNT_ID  = "SC_EURONETQA_DB" AND
CUM_AMOUNT_CNP_HOURLY > 100
)
)
)
...
DATA TXN_DATA;
SET TEMP_TXN_DATA;
WHERE FLAG = "Y" and date1 GE "10JUL2021"D;
RUN;

DATA RULE_DATA;
SET 
INC.falt003_db_10JUL
INC.falt003_db_11JUL;
WHERE UPCASE(RULE_NAME_STRG) IN ('MR_GLOBAL_STEAMGAMES_NON_MAJOR_TEST') and 
CRD_CLNT_ID NOT IN ("SC_SPARROWBD_DB" "SC_HOGANHK_DB" "SC_TANDEMTW_DB") and 
date1 GE "10JUL2021"D;
RUN;

PROC SORT DATA = RULE_DATA NODUPKEY; BY RULE_NAME_STRG FI_TRANSACTION_ID; RUN;

PROC FREQ DATA = TXN_DATA;TITLE "MR_Global_STEAMGAMES_Non_Major_Test  SAS";
TABLE CRD_CLNT_ID*DATE1/NOCOL NOROW NOPERCENT NOCUM; RUN; 

PROC FREQ DATA = RULE_DATA; TITLE "MR_Global_STEAMGAMES_Non_Major_Test  FALCON";
TABLE CLIENT_XID*DATE1/NOCOL NOROW NOPERCENT NOCUM; RUN; 

/* Writing Outout to Excel - For Audit*/

ods listing;
ods excel file='C:\Users\1510806\Desktop\SAS\Outputs\MR_Global_STEAMGAMES_Non_Major_Test Global Debit.xlsx'
...
outfile= "C:\Users\1510806\Desktop\SAS\Outputs\MR_Global_STEAMGAMES_Non_Major_Test Global Debit.xlsx"
dbms=xlsx replace;
sheet="TXN_DATA";
run;
