			...
			*******************************************************************
					 MR_Multi_Country_MID_Test Global Debit
...
(
(
COMPRESS(MER_ID) = "000445344340994" AND
 TRN_AMT  > 4000 AND
 FRD_SCOR  > 300
)
)
...
(
(
COMPRESS(UPCASE(MER_ID)) = "PG-GMSTR-E" AND
 TRN_AMT  >= 5000
) OR
(
...
)THEN MR_Multi_Country_MID_Test = "Y";
RUN;

DATA TXN_DATA;
SET TXN_DATA_SAS;
WHERE MR_Multi_Country_MID_Test = "Y" and date1 GE "02DEC2021"D;
RUN;
...
WHERE UPCASE(RULE_NAME_STRG) IN ('MR_MULTI_COUNTRY_MID_TEST') and date1 GE "02DEC2021"D;
RUN;

PROC SORT DATA=RULE_DATA NODUPKEY; BY RULE_NAME_STRG FI_TRANSACTION_ID; RUN;

PROC FREQ DATA=TXN_DATA;TITLE "MR_Multi_Country_MID_Test SAS";
TABLE CRD_CLNT_ID*DATE1/NOCOL NOROW NOPERCENT NOCUM; RUN; 
PROC FREQ DATA=RULE_DATA; TITLE "MR_Multi_Country_MID_Test FALCON";
...
ods listing;
ods excel file='C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\SAS\Outputs\MR_Multi_Country_MID_Test Global Debit.xlsx'
...
outfile= "C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\SAS\Outputs\MR_Multi_Country_MID_Test Global Debit.xlsx"
dbms=xlsx replace;
sheet="TXN_DATA";
run;
