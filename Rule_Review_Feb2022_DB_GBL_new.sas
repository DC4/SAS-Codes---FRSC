...
libname a "C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&MNTH00.";
...
%let rmth = 202202;
%let prev_mth = 202201;
%let prev_mth1 = 202112;
%let prev_mth2 = 202111;
%let rule_segment = a.RULE_SEGMENT;
%let disputefile = a.DISPUTE_MAR_2022_db;
%let portfolio = "DEBIT";
%let frd_table = a.FRAUD_DB_DEC21_TO_FEB22;
%let rule_hit_tab = a.RULE_HIT_LAST_3_MONTHS_DB;
%let rule_perf = a.RULE_PERFORMANCE_DB_MAR2022;
%let tran_tab = a.LAST_3_MONTHS_DATA_DB;

************************;

********** COMMENTS: Run Till new_rules dataset and check is there any new rule coming ********; 
...
CASE
WHEN SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2) NOT IN ("HK" "TW" "BD" "ID") THEN "Global" 
WHEN SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2) IN ("HK") THEN "HK"
WHEN SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2) IN ("TW") THEN "TW"
WHEN SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2) IN ("BD") THEN "BD"
WHEN SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2) IN ("ID") THEN "ID"
END AS TENANT
from &rule_hit_tab. ;
quit;

/*COMMENTS: if new_rule dataset contains any rule please go to the below path 
\\inhadfil101.in.standardchartered.com\wkgrps7\RAC_IN_MISS\Saravana\Monthly Reviews\Falcon
and update the file Rule_Segment_GBL_new.xls with the rule details*/
...
create a new file named dispute_transaction_(Curr_Mnth - 1)_gbl.xlsx in the below path*/
...
select distinct(acct_nbr ) as dispute_card from a.LAST_3_MONTHS_DATA_DB
...
data &frd_table.;
set &frd_table.;
country=SUBSTR(STRIP(CLIENT_XID),LENGTH(STRIP(CLIENT_XID))-4, 2);
...
IF CRD_CLNT_ID IN ("SC_CCMSSG_CR" "SC_CCMSBN_CR") THEN USD = TRN_AMT/1.255698;
	...
	ELSE IF CRD_CLNT_ID IN ("SC_C400JE_CR") AND TRN_CURR_CD = '840' THEN USD = TRN_AMT;
	ELSE IF CRD_CLNT_ID IN ("SC_C400JE_CR") AND TRN_CURR_CD = '978' THEN USD = TRN_AMT/0.85;
	ELSE IF CRD_CLNT_ID IN ("SC_C400JE_CR") AND TRN_CURR_CD = '826' THEN USD = TRN_AMT/0.75;


	IF CRD_CLNT_ID = "SC_EURONETAE_DB" THEN USD = TRN_AMT/3.67;
		ELSE IF CRD_CLNT_ID IN ("SC_SPARROWCI_DB") THEN USD = TRN_AMT/566;
		...
		ELSE IF CRD_CLNT_ID = "SC_SPARROWBD_DB" THEN USD = TRN_AMT/83.60;
...
tab78 tab79 tab80 tab81 tab38a tab46a tab54a tab79a tab80a tab81a
;
run;

data all1;
set all;
country=compress(tranwrd(tranwrd(scan(crd_clnt_id,2,"_"),"EURONET",""),"SPARROW",""));
keyfld=compress(tranwrd(tranwrd(scan(crd_clnt_id,2,"_"),"EURONET",""),"SPARROW",""))||put(frd_month,$6.)||var_name;
run;
...
proc sql;
create table a.DB_MID_Impact_Summary as 
...
proc sql;
create table a.rules_to_review_DB as 
select distinct(RULE_NAME_STRG) from REVIEW_DATA;
quit;

proc sql;
create table rule_analysis_data_DB1 as 
select * from &rule_perf.
where rule_name_strg in (select rule_name_strg from a.rules_to_review_DB);
quit;
...
/*select * from &tran_tab. where */
/*fi_transaction_id in (select fi_transaction_id from &rule_hit_tab. where RULE_NAME_STRG="&rulename.")*/
/*;*/
/*quit;*/
/**/
/*data rule1;*/
...
/*run;*/
/**/
/*proc append base=rule_analysis_data_DB1 data=rule1 force;run;*/
/**/
/*%mend;*/
/**/
/*proc sql noprint;*/
/*  select distinct RULE_NAME_STRG into : name_list separated by ' ' from a.rules_to_review_DB;*/
/*quit;*/
/**/
/*%MACRO GETDATA();*/
/*%local i next_name;*/
...
data rule_analysis_data_DB1;
set rule_analysis_data_DB1;
IF CRD_CLNT_ID IN ("SC_CCMSSG_CR" "SC_CCMSBN_CR") THEN USD = TRN_AMT/1.255698;
	ELSE IF CRD_CLNT_ID IN ("SC_CCMSMY_CR") THEN USD = TRN_AMT/3.221743;
	ELSE IF CRD_CLNT_ID IN ("SC_CCMSPH_CR") THEN USD = TRN_AMT/43.88082;
	ELSE IF CRD_CLNT_ID IN ("SC_CCMSTH_CR") THEN USD = TRN_AMT/32.675467;
...
proc import datafile="\\inhadfil101.in.standardchartered.com\wkgrps7\RAC_IN_MIS\Saravana\Monthly Reviews\Falcon\&MNTH1.\Debit\Data from fraudbase_DB_GBL_new.xlsx"
out= previous_month_db
dbms=xlsx replace;run;

data previous_month_db;
set previous_month_db;
...
data all4;
set previous_month_db all3;
run;

proc sql;
CREATE TABLE BLACKLIST_ANALYSIS AS 
select DISTINCT(FI_TRANSACTION_ID) AS TXNID from &rule_hit_tab. where RULE_NAME_STRG in ('HR_GLOBAL_BLACKLIST_MIDS_HOTLIST_DEBIT','HR_Global_Debit_Blacklist_MID');
quit;
...
proc export data=DISPUTE_MER_TXN1
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\DISPUTE_MER_TXN_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=all4 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\Data from fraudbase_DB_GBL_new.xlsx"
dbms=xlsx replace;run;

proc export data=RULE_SUMMARY outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\Overall rule analysis_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=UNDETECT_SUMMARY outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\Undetected case review_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=review_data outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\RUle review data_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=rule_analysis_data_DB1
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\rule_analysis_data_DB_GBL_new.xlsx"
dbms=xlsx replace;run;

proc export data=undetect_cases
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\undetect_cases_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=a.DB_MID_Impact_Summary
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\DB_MID_Impact_Summary_DB_GBL_new.xlsx"
dbms=xlsx replace;run;
proc export data=tab2x
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\tab2x_db.xlsx"
dbms=xlsx replace;run;
proc export data=BLACKLIST_ANALYSIS3
 outfile="C:\Users\1640982\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Consolidated Last 3 Months Data\&curr_mth.\BLACKLIST_ANALYSIS3_DB.xlsx"
dbms=xlsx replace;run;
