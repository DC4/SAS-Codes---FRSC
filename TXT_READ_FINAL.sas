...
PROC IMPORT DATAFILE="Z:\Saravana\Sparrow\Debit cards-block.xlsx"
OUT = DBCARDS;
RUN;

PROC SQL;
CREATE TABLE DBC AS
Select D.*, N.Status1 from DBCARDS D LEFT JOIN NN_ALL N ON
TRIM(D.ACCT_NBR) = TRIM(N.cardno);
RUN;

PROC EXPORT DATA=DBC OUTFILE="Z:\Saravana\Sparrow\RESULT.xlsx";
RUN;
