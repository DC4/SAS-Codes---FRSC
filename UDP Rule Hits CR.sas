...
LIBNAME CREDIT "C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Falcon 6.4 Monthly Data\credit";
...
PROC SORT DATA=UDP_CR_APR19_JUN19; BY UDP UDP_PROFILE; RUN;

DATA UDP_CR_APR19_JUN19_N;
LENGTH MERCHANTS $ 500. DECI_CD_ORIG1 $ 5.;
SET UDP_CR_APR19_JUN19;
RETAIN MERCHANTS;
BY UDP UDP_PROFILE;
...
PROC FREQ DATA=UDP_CR_APR19_JUN19; TABLE MONTH*MER_ID*DECI_CD_ORIG
/NOCOL NOCUM NOPERCENT NOROW MISSING OUT=MID_UDP_CR_APR19_JUN19; 
RUN;



PROC SQL;
CREATE TABLE UDP_CR_APR19_JUN19_SUMMARY AS 
...
FROM UDP_CR_APR19_JUN19_N
GROUP BY UDP, MONTH, COUNTRY, MER_ID, UDP_PROFILE;
QUIT;

PROC SORT DATA=UDP_CR_APR19_JUN19_SUMMARY; BY UDP MONTH COUNTRY MER_ID UDP_PROFILE DESCENDING MERCHANTS; RUN;
PROC SORT DATA=UDP_CR_APR19_JUN19_SUMMARY NODUPKEY; BY UDP MONTH COUNTRY MER_ID UDP_PROFILE; RUN;
...
PROC IMPORT DATAFILE="C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Required\Hotlist\BLACKLISTED_MERCHANTS_CREDIT_RAC_INDIA.xlsx" OUT=BM_CREDIT_RAC_INDIA DBMS=XLSX REPLACE;
RUN;
PROC IMPORT DATAFILE="C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Required\Hotlist\BLACKLISTED_MERCHANTS_CREDIT_RAC_SG.xlsx" OUT=BM_CREDIT_RAC_SG DBMS=XLSX REPLACE;
RUN;
PROC IMPORT DATAFILE="C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Required\Hotlist\BLACKLIST_MIDS_BINATTACK_CREDIT.xlsx" OUT=BINATTACK_CREDIT DBMS=XLSX REPLACE;
RUN;
PROC IMPORT DATAFILE="C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\Required\Hotlist\BLACKLIST_MIDS_COUNTRYWISE_CREDIT.xlsx" OUT=COUNTRYWISE_CREDIT DBMS=XLSX REPLACE;
RUN;
...
PROC SQL;
CREATE TABLE BM_CREDIT_RAC_INDIA1 AS
SELECT * FROM BM_CREDIT_RAC_INDIA WHERE NAME NOT IN (SELECT DISTINCT STRIP(MID) FROM BM0);
QUIT;
PROC SQL;
CREATE TABLE BM_CREDIT_RAC_SG1 AS
SELECT * FROM BM_CREDIT_RAC_SG WHERE NAME NOT IN (SELECT DISTINCT STRIP(MID) FROM BM0);
QUIT;

DATA BM1;
SET BM0(KEEP=MID BLACKLISTED_FOR)
	BM_CREDIT_RAC_INDIA1(RENAME=(NAME=MID COUNTRY=BLACKLISTED_FOR) KEEP=NAME COUNTRY)
	BM_CREDIT_RAC_SG1(RENAME=(NAME=MID COUNTRY=BLACKLISTED_FOR) KEEP=NAME COUNTRY);
...
PROC SQL;
CREATE TABLE BINATTACK_CREDIT1 AS
SELECT * FROM BINATTACK_CREDIT WHERE NAME NOT IN (SELECT DISTINCT MID FROM BM_1);
QUIT;
PROC SQL;
CREATE TABLE COUNTRYWISE_CREDIT1 AS
SELECT * FROM COUNTRYWISE_CREDIT WHERE NAME NOT IN (SELECT DISTINCT MID FROM BM_1);
QUIT;

DATA BM2;
SET BINATTACK_CREDIT1(RENAME=(NAME=MID VALUE=BLACKLISTED_FOR) KEEP=NAME VALUE)
	COUNTRYWISE_CREDIT1(RENAME=(NAME=MID VALUE=BLACKLISTED_FOR) KEEP=NAME VALUE);
RUN;
...
FROM UDP_CR_APR19_JUN19_SUMMARY) AS X
LEFT JOIN
BM AS Y
...
PROC EXPORT DATA=UDP_RULE_HITS_SUMMARY OUTFILE="C:\Users\1510806\OneDrive - Standard Chartered Bank\Desktop\Jhilam\CR UDP_RULE_HITS_SUMMARY.xlsx" DBMS=XLSX REPLACE;
RUN;